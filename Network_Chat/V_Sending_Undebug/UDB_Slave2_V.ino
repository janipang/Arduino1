...
void serialReceived(){
    ...
    if (inputStep == 2){ //สถานะ"ใส่เสร็จแล้ว" เราจะเอาข้อความมาประกอบ
        if (toWho >= 1 && toWho <= 3){
            serialBuffer = "token#S#N#"; //S N have no meaning i just only random it
            serialBuffer[6] = '0' + 1;
            serialBuffer[8] = '0' + toWho;
            serialBuffer += text;
            serialBuffer += '\0';
            inputStep = (inputStep + 1) % 3; //we have input 3 steps : 1.input num 2.inout text 3.put it together
        }
        else{
        ...
        //Serial.print("SerialBuffer >>"); //for debugging
...
//----------------------------------------------------------------------
    ...
    if (debugMode == 1){ //T. required us to have debug mode that show every token that have transmitted
        ...
        delay(2500); //use this value bc. its so fast that i cant find the error :but gym can change it na
    }
